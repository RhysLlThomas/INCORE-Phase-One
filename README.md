...
This script reads in cleaned data, creates a set of regression matrices, and saves the resulting data to a set of directories within the  `03_prepped_inputs` directory. As before, this script **should NOT** require any modification, since the input data should be standardized.
...
Finally, this script runs LASSO regression using 5-fold cross validation on the data from `data/03_prepped_inputs`. Regressions are run using the `h2o` library, which, again, is used to assist with memory management.

`h2o` has to load the data before it can run a regression, which can take a bit of time. Fitting the regression model itself is fairly fast, and the resulting coefficients are saved to the `results` folder.
